# maybe someday we can adjust this to use
# following http://cran.r-project.org/web/packages/bestglm/vignettes/bestglm.pdf
# I have the beginnings of code for that in: Script_regression-bestglm-6.R

# note this code:
#thesefitinresponseunits = exp(cc2$se.fit)*1.96
# IT IS AN ATTEMPT AT A PREDICTION INTERVAL.

#look here:
#  https://stat.ethz.ch/pipermail/r-help/2010-September/254465.html

#and here
#http://twri.tamu.edu/bret/BretWebSiteDocs/GLMCI.pdf

# but also here, where I think the implication is that I am probably doing this wrong
...
predictionTableFilename <- args[2]
txtOutput <- args[3]
csvModelOutput <- args[4]
pdfOutput <- args[5]
csvPredOutput <- args[6]

regressionTable = read.table(regressionTableFilename, header=T, sep=",")
predictionTable = read.table(predictionTableFilename, header=T, sep=",")
predictionTable$Rowid_ <- NULL
predictionTable$ZONE_CODE <- NULL
predictionTable$STD <- NULL
predictionTable$RANGE <- NULL
predictionTable$COUNT <- NULL
regressionTable$Rowid_ <- NULL
regressionTable$ZONE_CODE <- NULL
regressionTable$STD <- NULL
regressionTable$RANGE <- NULL
regressionTable$COUNT <- NULL
predictionTable$SCENEID <- NULL
regressionTable$SCENEID <- NULL
predictionTable$CHL <- NULL
regressionTable$CHL <- NULL
predictionTable$COUNT_CDOM <- NULL
regressionTable$COUNT_CDOM <- NULL
						 
regressionNIDs = regressionTable[[1]]
regressionCDOM = regressionTable[[2]]
regressionBands = regressionTable[[3]]

predictionNIDs = predictionTable[[1]]
predictionBands = predictionTable[[2]]

						 
thelakedata = data.frame(regressionNIDs,regressionCDOM,regressionBands)
thelakepred = data.frame(predictionNIDs,predictionBands)


print(summary(thelakedata))
print(thelakedata)
print(summary(thelakepred))
...
afn.singlemodel.predict.withpredictionintervals = function(themodel, thexvalue)
{
  ...
  theret = data.frame(thepred.responseunits, predictioninterval)
  return(theret)
}
...
thepred = theret[,1]
predictioninterval.ithink = theret[,2] # note we had multiplied the se.fit by exp(se.fit)*1.96
groundcarbonvalue = thedependent
imageryvalue = theindependent
predictions.forfusion = (data.frame(regressionNIDs, imageryvalue, groundcarbonvalue, thepred, predictioninterval.ithink))
...
#####Change independent to full set
theindependent = predictionBands

theret = afn.singlemodel.predict.withpredictionintervals(chosenmodel, theindependent)
thepred = theret[,1]
imageryvalue = theindependent
predictioninterval.ithink = theret[,2] # note we had multiplied the se.fit by exp(se.fit)*1.96
predictions.forfusion = (data.frame(predictionNIDs, imageryvalue, thepred, predictioninterval.ithink))
write.table(predictions.forfusion, csvPredOutput, row.names= F, quote = F, sep=",")
